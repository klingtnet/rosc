...
#[test]
fn test_encode_tcp_message_wo_args() {
    let msg_packet = OscPacket::Message(OscMessage {
        addr: "/some/addr".to_string(),
        args: vec![],
    });
    ...
    let dec_msg = match decoder::decode_tcp(&enc_msg).unwrap().1 {
...
#[test]
fn test_encode_tcp_empty_bundle() {
    let bundle_packet = OscPacket::Bundle(OscBundle {
        timetag: (4, 2).into(),
        content: vec![],
    });
...
#[test]
fn test_encode_tcp_message_with_args() {
    let msg_packet = OscPacket::Message(OscMessage {
        addr: "/another/address/1".to_string(),
        args: vec![
            4i32.into(),
